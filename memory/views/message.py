...
from memory.models import Group, Student, Teacher, Mentor, Waiter
from django.core.exceptions import ObjectDoesNotExist
from memory.helpers import get_redir_url,ajax_ok
...
def message_history(request, username, page=1, paginate_by=10, compose_form=ComposeForm,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            #actions = {'title':'新消息','href':redirect_to}
            #notify.send(request.user, verb='新消息', action_object=message, recipient=recipient, actions=actions)
...
def message_quick_contact(request, template_name="umessages/message_quick_contact.html"):
    ...
    try:
        ...
        if student:
            group = student.group
            teachers = group.teachers.all().select_related('user')

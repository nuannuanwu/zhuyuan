...
from memory.models import Mentor,ApplePushNotification,Device,Tile,Sms,Cookbook,CookbookRead,Student,Access_log,RelevantStaff
...
from memory.settings import DATABASES
...
def push_unread_message(request): 
    ...
    send_time = datetime.datetime.now() + datetime.timedelta(seconds = -300)
    sql = "select a.id,a.user_id,a.message_id,b.token from umessages_messagerecipient a  left join memory_device b on a.user_id = b.user_id  left join umessages_message c on a.message_id = c.id where c.sent_at<'" + str(send_time) + "' and a.no_need_send=0 and a.is_push =0 and a.read_at is null and a.deleted_at is null and b.token is not null limit 0,1000"
...
def send_emergency_message(request): 
    ...
    sql = "select a.id,a.user_id,a.message_id,c.sender_id,c.body from umessages_messagerecipient a  left join umessages_message c on a.message_id = c.id where c.type=2 and c.sent_at>'" + str(time_start) + "' and c.sent_at<'" + str(time_end) + "' and a.no_need_send=0 and a.is_send =0 and a.read_at is null limit 0,1000"
...
def send_user_message(request): 
    ...
    has_send = Sms.objects.filter(type_id=98,send_time__gt=time_limit)
    if has_send:
        result = "total push user_message:"+str(count)
        return HttpResponse(result)
    
    sql = "SELECT user_id,MAX(send_time) stime FROM `memory_access_log` GROUP BY user_id HAVING stime < '" + str(time_limit) + "'"
    
    cursor = connection.cursor()
    cursor.execute(sql)
    desc = cursor.description
    result_list = [dict(zip([col[0] for col in desc], row)) for row in cursor.fetchall()]
    user_pks = [r['user_id'] for r in result_list]

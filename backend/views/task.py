...
from memory.models import Char,Teacher, Student, School,Access_log,Tile,Activity,TileToActivity,Cookbook,CookbookRead,CookbookType,EventType,Group
...
def parent_info(request,template_name="backend/parent_info.html"):
    ...
    `user`.date_joined FROM `memory_school` AS school LEFT JOIN `memory_group` AS class ON class.`school_id` = school.`id` \
    LEFT JOIN `memory_student` AS student ON student.`group_id` = class.`id` LEFT JOIN `auth_user` AS `user` ON \
